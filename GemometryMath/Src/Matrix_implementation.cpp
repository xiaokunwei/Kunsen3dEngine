...
#define d r22
	...
	if (KunSen::square(d - 1.0) > 1.0e-6)  // Involves perspective, so we must
...
void Matrix_implementation::getLookAt(Vec3f& eye,Vec3f& center,Vec3f& up,value_type lookDistance) const
{
	...
	Vec3f e = KunSen::Vec3f(0.0, 0.0, 0.0)*inv;
	up = transform3x3(*this, KunSen::Vec3f(0.0, 1.0, 0.0));
	Vec3f c = transform3x3(*this, KunSen::Vec3f(0.0, 0.0, -1));
    c.normalize();
    c = e + c*lookDistance;
...
void Matrix_implementation::getLookAt(Vec3d& eye,Vec3d& center,Vec3d& up,value_type lookDistance) const
{
    ...
    // note: e and c variables must be used inside this method instead of eye and center
    // because eye and center are references and they may point to the same variable.
	Vec3d e = KunSen::Vec3d(0.0, 0.0, 0.0)*inv;
	up = transform3x3(*this, KunSen::Vec3d(0.0, 1.0, 0.0));
	Vec3d c = transform3x3(*this, KunSen::Vec3d(0.0, 0.0, -1));
    c.normalize();
    c = e + c*lookDistance;
